            ...
            repeat
              if CheckFileName(searchResult.Name) = True then
                begin
                  ...
                  addLog('Переносим файл ' + searchResult.Name + ' в директорию для отправки письма VipNet''ом: ' + fileDirectoryToVipnet, isInformation);
...
begin
  ...
  if MatchesMask(inputFileName, 'SH_*_*_*.zip') or
     ...
     MatchesMask(inputFileName, 'MSH_*_MTP_*.zip') or //MTP – по-английски
     MatchesMask(inputFileName, 'MSMP_*_МТР_*.zip') or //MTP – по-русски
     MatchesMask(inputFileName, 'MSMP_*_MTP_*.zip') or //MTP – по-английски
     MatchesMask(inputFileName, 'CON_*_МТР_*.zip') or //MTP – по-русски
     MatchesMask(inputFileName, 'CON_*_MTP_*.zip') or //MTP – по-английски
     MatchesMask(inputFileName, 'RCON_*_МТР_*.zip') or //MTP – по-русски
     MatchesMask(inputFileName, 'RCON_*_MTP_*.zip') then //MTP – по-английски
    begin
      Result := True;
    end;
...
procedure TFormMain.AddLog(inputString: string; LogType: integer); //LogType бывает:
                                                                   //isError – цвет текста Красный
                                                                   //isSuccess – цвет текста Чёрный, Курсив
                                                                   //isInformation – цвет текста Чёрный
begin
  case LogType of
    isError: begin
...
begin
  statusbarProcessing.Panels.Items[0].Text := 'Отправлено ' + IntToStr(statusSentMailsCount) + ' писем из ' + IntToStr(StatusTotal);
end;
